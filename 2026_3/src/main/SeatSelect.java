...
public class SeatSelect extends CFrame{
	...
	SeatSelect(Data data){
		...
		setFrame("좌석 예매", 710, 600);
	}
	
	private void setSouthPanel() {
		JLabel img = new JLabel(getter.getImageIcon("datafiles/movies/" + movie.getInt(0) + ".jpg", 110, 145));
		...
		JLabel date = new JLabel("일시: " + data.get(3));
		date.setFont(font);
	...
	private void setAction() {
		...
		List<Data> list = Connections.select("SELECT r_setname FROM moviedb.reservation where m_no = ? and r_time = ? and r_date = ?;", data.get(1), data.get(4), data.get(3));
		for(int i = 0; i < panels.size(); i++) {
			...
			panels.get(index).addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					for(Data seatName : list) {
						if(seatName.get(0).toString().equals(seat)) {
							return;
						}
					}
