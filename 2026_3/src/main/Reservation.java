...
public class Reservation extends CFrame{
	...
	LocalDate nows = LocalDate.of(2026, 4, 9);
	...
	private void setMainPanel() {
		...
		mainPanel.setLayout(new GridLayout(list.size() < 4 ? 4 : list.size(), 1, 10, 10));
	...
	private void setWPanel() {
		...
		for(int i = 1; i <= nows.lengthOfMonth(); i++) {
			...
			if(!date.isBefore(nows)) {
				...
				p.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						getter.mg("현재시간 이후만 선택가능", JOptionPane.ERROR_MESSAGE);
					}
				});
			}
			gridPanel.add(p);
