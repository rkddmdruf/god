...
public class HRSystem {
    ...
    //  상수 & 공통 데이터
    // ══════════════════════════════════════════════
    static final String[] TEAM_NAMES  = {"개발", "영업", "관리", "생산"};
    ...
    static void showAttRegister(JFrame parent){
        ...
        JTextField txtDate=makeROTF(LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
        String[] types={"지각","결근"};
        JComboBox<String> cmbType=makeCmb(types);

        Object[][] rows={{"직원 선택 *",cmbEmp},{"근무 일자 *",txtDate}, {"근무 유형 *",cmbType}};
        for(int i=0;i<rows.length;i++){
            addGBC(body,makeLbl((String)rows[i][0]),gc,0,i,1,0);
            addGBC(body,(Component)rows[i][1],gc,1,i,1,1);
        }
        JPanel bp=new JPanel(new FlowLayout(FlowLayout.LEFT,8,0)); bp.setOpaque(false);
        JButton btnSave=makePBtn("저장"),btnList=makeSBtn("근태 현황 보기");
        bp.add(btnSave); bp.add(btnList);
        addGBC(body,bp,gc,0,rows.length,2,1);
    ...
    static void showAttList(JFrame parent){
        ...
        sp.add(makeLbl("직원:")); sp.add(cmbEmp); sp.add(btnSearch);
        sp.add(Box.createHorizontalStrut(10)); sp.add(lblSummary);

        String[] cols={"근태ID","사원코드","이름","날짜","유형"};
        DefaultTableModel model=new DefaultTableModel(cols,0){public boolean isCellEditable(int r,int c){return false;}};
        JTable table=makeTable(model);
        int[] ws={80,90,70,100,70}; for(int i=0;i<ws.length;i++) table.getColumnModel().getColumn(i).setPreferredWidth(ws[i]);

        Runnable reload=()->{
            model.setRowCount(0);
            int empIdx=cmbEmp.getSelectedIndex()-1;
            int n=0,l=0,et=0,ab=0;
            List<Data> list = Connections.select("SELECT code, name, commute_time.date, status FROM emp.commute_time\r\n"
            		+ "join emps on emps.eno = commute_time.eno;");
            for(Data a: list){
            	if(cmbEmp.getSelectedIndex() != 0 && !cmbEmp.getSelectedItem().toString().contains(a.getString(0))) continue;
                model.addRow(new Object[]{n,a.getString(0), a.getString(1), a.getString(2), a.getInt(3) == 0 ? "지각" : "결근"});// 테이블 추가
                n++; switch(a.getInt(3)){case 0:l++;break;case 1:et++;break;}
            }
            lblSummary.setText(String.format("지각 %d · 결근 %d",l,et));
        };
    ...
    static void showSalaryList(JFrame parent){
        ...
        List<String> yearList=new ArrayList<>(); yearList.add("올해 전체");
        ...
        String[] cols={"사원코드","이름","년월","기본급","수당","세금(15%)","실수령액"};
        DefaultTableModel model=new DefaultTableModel(cols,0){public boolean isCellEditable(int r,int c){return false;}};
        JTable table=makeTable(model);
        ...
        Runnable reload=()->{
            ...
            int empIdx=cmbEmp.getSelectedIndex()-1;
            int yearSel=cmbYear.getSelectedIndex();
            List<Data> list = Connections.select("select emps.code, name, payList.date, pay, otPay, taxPay  from payList join emps on emps.eno = payList.eno");
            for(Data s: list){
                if(empIdx>=0){
                    String selCode=opts.get(cmbEmp.getSelectedIndex()).split(" ")[0];
                    if(!s.getString(0).equals(selCode)) continue;
                }
                if(yearSel != 0 && LocalDate.parse(s.getString(2)).getMonthValue() >= yearSel) continue;
                model.addRow(new Object[]{s.getString(0),s.getString(1),s.getString(2),s.getString(3),s.getInt(4),s.getString(5),s.getInt(4) + s.getInt(3) - s.getInt(5)});
                totalNet+=s.getInt(3) + s.getInt(4) - s.getInt(5);
            }
        ...
        JPanel bp=new JPanel(new FlowLayout(FlowLayout.LEFT,8,6)); bp.setBackground(new Color(250,251,255));
        bp.setBorder(BorderFactory.createMatteBorder(1,0,0,0,C_BORDER));
        JButton btnCalc=makeSBtn("급여 계산 이동"), btnBack=makeSBtn("메인으로");
        bp.add(btnCalc); bp.add(btnBack);
    ...
    static void showStats(JFrame parent){
        ...
        List<String> ymList=new ArrayList<>() {{
        	for(int i = 1; i <= LocalDate.now().getMonthValue(); i++) {
        		add(LocalDate.of(LocalDate.now().getYear(), i, 1).format(DateTimeFormatter.ofPattern("yyyy-MM")));
        	}
        }};
        ...
        Runnable reload=()->{
            ...
            List<Data> list = Connections.select("SELECT * FROM emp.emps join paylist on paylist.eno = emps.eno where paylist.date between ? and ?;", LocalDate.now().withDayOfMonth(1), LocalDate.now().withDayOfMonth(LocalDate.now().lengthOfMonth()));
            System.out.println(list);
            int cnt=list.size(); double sum=list.stream().mapToInt(e -> e.getInt(e.size() - 3) + e.getInt(e.size() - 2) - e.getInt(e.size() - 1)).sum(); double avg=sum/cnt;
            ...
            for(int t=0;t<TEAM_NAMES.length;t++){
                final int team=t;
                int teamSize = list.stream().filter(e -> e.getInt(3) == team).collect(Collectors.toList()).size();
                int sumPrice = list.stream().filter(e -> e.getInt(3) == team).mapToInt(e -> e.getInt(e.size() - 3) + e.getInt(e.size() - 2) - e.getInt(e.size() - 1)).sum();
                int avgPrice = sumPrice / list.stream().filter(e -> e.getInt(3) == team).collect(Collectors.toList()).size();
                List<Data> sortList = list.stream().filter(e -> e.getInt(3) == team).collect(Collectors.toList());
                sortList.sort((a, b) -> 
                Integer.valueOf(
                    a.getInt(a.size() - 3) + 
                    a.getInt(a.size() - 2) + 
                ...
                Data data = sortList.get(0);
                int hiPrice = data.getInt(data.size() - 3) + data.getInt(data.size() - 2) - data.getInt(data.size() -1);
                sortList.sort((a, b) -> 
	                Integer.valueOf(
	                    b.getInt(b.size() - 3) + 
	                    b.getInt(b.size() - 2) + 
                ...
                data = sortList.get(0);
                int loPrice = data.getInt(data.size() - 3) + data.getInt(data.size() - 2) - data.getInt(data.size() -1);
                ...
                deptModel.addRow(new Object[]{TEAM_NAMES[t],teamSize, sumPrice, avgPrice, hiPrice, loPrice});
            }
        };
        ...
        // 레이아웃 조립
        JPanel tables=new JPanel(new GridLayout(1,1,0 ,0));
