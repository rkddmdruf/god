...
where m_no = ?
	...
	private void setChat() {
		...
		for(int i = 0; i < rList.size(); i++) {
			...
			p.add(new JLabel(getter.getImage("datafiles/user/" + rList.get(i).getInt(0) + ".jpg", 50, 50)), BorderLayout.WEST);
			p.add(ip);
	...
	private void setNorthPanel() {
		...
		JLabel img = new JLabel(getter.getImage("datafiles/movies/" + m_no + ".jpg", 160, 240)) {
			@Override
			protected void paintComponent(Graphics g) {
				super.paintComponent(g);
