...
public class C_Detail extends BaseFrame{
	...
	public void desgin() {
		...
		for(int i = 0 ; i < check.size(); i++) {
			if(check.get(i).get(7) == null) {check.get(i).clear();//check는 판매수량 순서 카테고리 별
			}else if(check.get(0).getInt(7) != check.get(i).getInt(7)) {check.get(i).clear();}//가장위는 판매수량이 많은거 순서대로 조회해서 첫번쨰보다 작으면 삭제
		}
		add(new JPanel(new BorderLayout()) {{
			setBackground(Color.white);setBorder(BorderFactory.createEmptyBorder(10,10,10,10));
			add(new JLabel() {{
			...
			for(int i = 0 ; i < check.size(); i++) {
				if(check.get(i).isEmpty()) {break;}
				if(list.get(0).getInt(0) == check.get(i).getInt(0)) {
					System.out.println("높음");
					add(new TrianglePanel(Query.getImge.getImge(product)) {{setPreferredSize(new Dimension(265, 180));}}, sp.n);break;
				}
			}
			add(new JPanel(new GridLayout(6,1)) {{
				...
				add(new JTextArea("설명 : " + c) {{setFont(sp.fontM(1,12));setEditable(false);}});
			...
			add(new JPanel(new GridLayout(0,2, 10, 0)) {{
				setBackground(Color.white);
				for(JButton j : but) { j.setPreferredSize(new Dimension(0, 35)); add(j); }
			}},BorderLayout.SOUTH);
		}}, BorderLayout.CENTER);
	}
...
class TrianglePanel extends JPanel {
	Image img = null;
    public TrianglePanel(Image img) {	
		this.img = img;
	}
	@Override
	public void paint(Graphics g) {
        ...
        g.drawImage(img, 0,0, 265,180,null);
        g.drawRect(0,0, 263, 179);
