...
public class AMain extends BaseFrame{// 검색 위에 기능 만들기
	...
	D_Cart2 d = new D_Cart2();
	E_GumeList e = new E_GumeList();
	...
	public void desgin() {
		...
		add(new JPanel(new BorderLayout()) {{
			...
			add(new JPanel(new BorderLayout()) {{
				setBackground(Color.white);
				Login.setIcon(new ImageIcon(new ImageIcon("src/logo/유저.png").getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH)));
		...
		new HOME(this,cardP, user);
		if(Serchs == -1) { 
			new Serch(this,cardP, user, 0); 
		}else {
			new Serch(this,cardP, user, Serchs); 
			card.show(cardP,"P2"); 
		}
		
		if(user != 0 && user != 1119) {
			d= new D_Cart2(cardP, user);
			e =new E_GumeList(cardP, user);
		...
		serch.butB[0].addActionListener(e->{
			...
			if(serch.cb.getSelectedIndex() == 0) {
				if(serch.product.getText().equals("")) {
					...
					sp.ErrMes("검색 결과가 없습니다.");serch.product.setText("");
					serch.All();serch.category();serch.card.show(serch.cardP, "P0");repaint();revalidate();
				}else {
					List<Row> list = Query.productSerchisEm.select(serch.product.getText());
					for(int i = 1; i <= 10; i++) {
						serch.removes(i);
					}
					JPanel serchP = new JPanel(new GridLayout(0, 4 ,20, 10));serchP.setBorder(BorderFactory.createEmptyBorder(0,20,0,10));
					serch.butS = new ArrayList<>();
					for(int i = 0; i < list.size(); i++) {
						serch.global = list.get(i).getInt(0);
						serch.panelAdd(serchP, serch.butS, i+1);
					}
					if(list.size() < 4){for(int i = list.size(); i <= 4; i++){serchP.add(new JLabel(" "));}}
					serch.cardP.add(serch.scMain = new JScrollPane(serchP), "P0");
					serch.scMain.getVerticalScrollBar().setUnitIncrement(20);
					serch.card.show(serch.cardP, "P0");
			...
			if(serch.cb.getSelectedIndex() == 1) {
				try {
					...
					if(Query.productPriceMxMn.select(first, last).isEmpty()) {
						sp.ErrMes("검색 결과가 없습니다.");serch.price[0].setText("");serch.price[1].setText("");
					}else {
						List<Row> list =  Query.productPriceMxMn.select(first, last);
						for(int i = 1; i <= 10; i++) {
							serch.removes(i);
						}
						JPanel serchP = new JPanel(new GridLayout(0, 4 ,20, 10));serchP.setBorder(BorderFactory.createEmptyBorder(0,20,0,10));
			...
			but[i].addActionListener(e->{
				if(user <= 0) {
					...
					if(e.getSource() == but[3]) {
						this.e = new E_GumeList(cardP, user);
					}else if(e.getSource() == but[4]) {
						this.f = new F_Roupang(cardP, user);
					}
					card.show(cardP, "P" + (index+1));
