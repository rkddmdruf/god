		...
		add(new JPanel(new GridLayout(1, 2)) {{
			...
			if(lt.isEmpty()) {
				...
				for(Row list : lt) {
					...
					JP.get(global).add(new JLabel(new ImageIcon(Query.getImge.getImge(list.getInt(2)).getScaledInstance(150, 100, Image.SCALE_SMOOTH))) {{
	...
	private void action(List<JPanel> P, List<JCheckBox> CKB, List<Row> list) {
		...
		for(JButton b : but) {
			b.addActionListener(e->{
				...
				if(number == CKB.size()) {
					if(b == but[0]) JOptionPane.showMessageDialog(this, "삭제할 상품을 선택하세요.", "경고", JOptionPane.ERROR_MESSAGE);
					if(b == but[1]) JOptionPane.showMessageDialog(this, "구매할 상품을 선택하세요", "경고", JOptionPane.ERROR_MESSAGE);
					...
					for(int i = data.size()-1; i >= 0; i--) {//// 선택상품 사이즈 -1(두개면 2-1 = 1 -> for돌때 1, 0두번 돔)
						...
						for(int j = CKBS; j >= 0; j--) {//뒤에서 부터 삭제
							if(data.get(i) == j) {
								if(Query.cart개수.select(list.get(j).getString(2)).get(0).getInt(0)-list.get(j).getInt(3) < 0 && b == but[1]){
