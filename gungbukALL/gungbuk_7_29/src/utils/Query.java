...
public enum Query {
	...
	productAll("SELECT product.*, sum(`order`.quantity) FROM roupang.product LEFT JOIN `order` ON product.pno = `order`.pno where product.pno = ?;"),
	...
	productinsert("insert into product values(0,?,?,?,?,?,?);"),
