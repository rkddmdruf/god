...
public class GameInfor extends CFrame{
	...
	public GameInfor(int g_no) {
		if(LocalDate.parse(user.getU_bd().toString()).isAfter(LocalDate.now().minusYears(19))) {
		...
		game = Entity2.select(Gameinformation.class, Nev.avg(Comments.C_STAR.getNev(), "z"), Category.CA_NAME.getNev())
				.from(Gameinformation.class)
				.rightJoin(Comments.G_NO.getNev(), Category.CA_NO.getNev())
				.where(Gameinformation.G_NO.eq(g_no))
