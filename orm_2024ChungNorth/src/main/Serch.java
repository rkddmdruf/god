...
public class Serch extends CFrame {
	...
	JComboBox<Object> category = new JComboBox<Object>() {{
		for (Category data : Entity.findAll(Category.class))
			addItem(data.getCa_name());
	}};
			...
			imgList.get(i).addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(isInField(e.getPoint())) {
						...
						int uno = UserU.getUser().getU_no(), gno = data.getInt(0);
						...
						/*List<Data> buyGame = Connections.select("SELECT * FROM game_site.purchasegame \r\n"
								+ "where u_no = ? and g_no = ?;", uno, gno);*/
						List<Shoppingbasket> inBasket = Entity.where(Shoppingbasket.class, Shoppingbasket.U_NO.eq(uno), Shoppingbasket.G_NO.eq(gno));
						if(!inBasket.isEmpty()) {
							getter.mg("장바구니에 있는 게임입니다.", JOptionPane.ERROR_MESSAGE);
							return;
						}
						List<Purchasegame> buyGame = Entity.where(Purchasegame.class, Purchasegame.U_NO.eq(uno), Purchasegame.G_NO.eq(gno));
						if(!buyGame.isEmpty()) {
						...
						s.setP_no(0);   s.setG_no(gno);
						s.setU_no(uno); s.insert();
					}
