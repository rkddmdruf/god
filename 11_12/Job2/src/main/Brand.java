...
public class Brand extends BaseFrame{
	...
	public void setbrandPanels() {
		...
		brand = Query.Brand_getBrand.select(cno[0], cno[1]);
		...
		for(int i = min; i < (brand.size() < (max + 1) * 6 ? brand.size() : (max + 1) * 6); i++) {
			brandPanel.add(new sp.cl(sp.getImg("brand/" + brand.get(i).getInt(0) + ".png", 100, 100)).setBorders(sp.line));
