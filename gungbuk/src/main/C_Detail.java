	...
	public void desgin() {
		...
		add(new JPanel(new BorderLayout()) {{
			...
			add(new JLabel() {{
				...
				setIcon(new ImageIcon(new ImageIcon(getClass().getResource("/img/"+product+".jpg")).getImage().getScaledInstance(265, 180, Image.SCALE_SMOOTH)));
			}}, BorderLayout.NORTH);
			...
			add(new JPanel(new GridLayout(6,1)) {{
				...
				add(new JTextArea("설명 : " + c) {{setFont(setBoldFont(12));}});
	...
	public void action() {
		for(JButton b : but) {
			b.addActionListener(e->{
				if(e.getSource() == but[0] && true) {
					try {
						Integer j = Integer.parseInt(number.getText());
						JOptionPane.showMessageDialog(this, "장바구니에 등록되었습니다.", "정보", JOptionPane.INFORMATION_MESSAGE);
